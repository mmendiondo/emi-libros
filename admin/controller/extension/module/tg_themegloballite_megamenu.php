...
class ControllerExtensionModuleTGthemegloballiteMegamenu extends Controller {
	...
	public function index() {  
		...
		$this->language->load('extension/module/tg_themegloballite_megamenu');
		$this->document->setTitle('TG Themeglobal Pro Megamenu'); 
		
		$this->load->model('setting/setting');
		
		$this->document->addStyle('view/stylesheet/tg_themegloballite_megamenu.css');
		$this->document->addScript('view/javascript/jquery/jquery.nestable.js');
		
		$this->load->model('menu/tg_themegloballite_megamenu');
		
		$this->model_menu_tg_themegloballite_megamenu->install();
		...
		if(isset($_GET['delete'])) {
			if($this->validate()){
				if($this->model_menu_tg_themegloballite_megamenu->deleteMenu(intval($_GET['delete']))) {
					$this->session->data['success'] = 'This menu has been properly removed from the database.';
				} else {
					$this->session->data['error_warning'] = $this->model_menu_tg_themegloballite_megamenu->displayError();
				}
			} else {
				$this->session->data['error_warning'] = $this->language->get('error_permission');
			}
			$this->response->redirect($this->url->link('extension/module/tg_themegloballite_megamenu', 'token=' . $this->session->data['token'], true));
		}
			...
			if(isset($_POST['button-create'])) {
				if($this->validate()) {
					...
					if($error == true) {
						...
						$this->model_menu_tg_themegloballite_megamenu->addMenu($this->request->post);
			...
			if(isset($_POST['button-edit'])) {
				if($this->validate()) {
					...
					if($error == true) {
						...
						$this->model_menu_tg_themegloballite_megamenu->saveMenu($this->request->post);
						$this->session->data['success'] = $this->language->get('text_success');
					}
				} else {
					$this->session->data['error_warning'] = $this->language->get('error_permission');
		...
		$data['nestable_list'] = $this->model_menu_tg_themegloballite_megamenu->generate_nestable_list($data['language_id']);
				
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(isset($_POST['button-save'])){
				$tg_themegloballite_megamenu = array();
				...
				$tg_themegloballite_megamenu['tg_themegloballite_megamenu_module'] = array(
					array(
				...
				$this->model_setting_setting->editSetting('tg_themegloballite_megamenu', $tg_themegloballite_megamenu);
				$this->session->data['success'] = $this->language->get('text_success');
				$this->response->redirect($this->url->link('extension/module/tg_themegloballite_megamenu', 'token=' . $this->session->data['token'], true));	
			}
		}
		...
		if (isset($_GET['jsonstring'])) {
			if($this->validate()){
				...
				foreach ($readbleArray as $key => $value) {
					if (is_array($value)) {
						$this->model_menu_tg_themegloballite_megamenu->save_rang($value['parentID'], $value['id'], $key);
					}	
				}
		...
		if($this->config->get('tg_themegloballite_megamenu_module') != '') {
			$settings = $this->config->get('tg_themegloballite_megamenu_module');
			...
			$data['navigation_text'] = $settings[0]['navigation_text'];
		...
		if(isset($_GET['edit'])) {
			...
			$dane = $this->model_menu_tg_themegloballite_megamenu->getMenu(intval($_GET['edit']));
			if($dane) {
				$data['name'] = $dane['name'];
				...
				$data['list_categories'] = $this->model_menu_tg_themegloballite_megamenu->getCategories($dane['content']['categories']['categories']);
			} else {
		...
		$data['action'] = $this->url->link('extension/module/tg_themegloballite_megamenu', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'text' => 'TG themegloballite MegaMenu',
			'href' => $this->url->link('extension/module/tg_themegloballite_megamenu', 'token=' . $this->session->data['token'], true)
		);
		...
		$this->response->setOutput($this->load->view('extension/module/tg_themegloballite_megamenu', $data));
	
	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/tg_themegloballite_megamenu')) {
